...
public class implBiodata implements interBiodata {
    ...
    public void insert(modelBiodata m) {
        ...
        try {
            ...
            ps.setString(4, m.getAsal());
            ps.setDate(5, (Date) m.getTanggalLahir());
            ps.execute();
        } catch (Exception e) {
    ...
    public void update(modelBiodata m) {
        ...
        PreparedStatement ps;
        try {
            ps = k.connect().prepareStatement("update tablebiodata set nama=?, alamat=?, asal=?,tanggalLahir=? where id = ?");
            ps.setString(5, m.getId());
            ps.setString(1, m.getNama());
            ps.setString(2, m.getAlamat());
            ps.setString(3, m.getAsal());
    ...
    public ObservableList<modelBiodata> getAll() {
        ...
        try {
            ...
            while (rs.next()) {   
                ...
                m.setAsal(rs.getString(4));
                m.setTanggalLahir(rs.getDate(5));
                listData.add(m);
            }
        } catch (Exception ex) {
